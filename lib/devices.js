...
const NETWORK_FS = new Set(['smbfs', 'afpfs', 'nfs', 'webdav', 'cifs']);

/** macOS: mount 出力からネットワーク FS のマウント先集合を得る (NAS 判定用) */
    ...
    execFile('mount', (err, stdout) => {
      ...
      if (!err) {
        for (const line of String(stdout).split('\n')) {
          ...
          if (m && NETWORK_FS.has(m[2])) set.add(m[1]);
        }
      }
      resolve(set);
    });
    ...
    for (const name of names) {
      ...
      try {
        ...
        if ((await stat(p)).isDirectory()) {
          // kind: NAS などのネットワークマウントか、SD/USB 等のローカルか
          out.push({ path: p, label: name, kind: net.has(p) ? 'network' : 'removable' });
        }
      } catch {
        // アンマウント中など: スキップ
      }
