...
// ルートが違えば同じ相対パスでも別 ID になるよう、ルートを含めてハッシュする
// (メインライブラリと USB デバイスなど複数ソースを同時に扱うため)
function trackId(rootDir, relPath) {
  return createHash('sha1')
    .update(path.resolve(rootDir) + '\x00' + relPath)
...
export const IGNORED_DIRS = new Set([
  '@recycle', '@recently-snapshot',      // QNAP
  '#recycle', '#snapshot',               // Synology
...
/** 指定ルートのスキャンキャッシュを削除する (固定解除時のプライバシー対応) */
export async function deleteLibraryCache(rootDir) {
  try {
    await rm(cacheFileFor(rootDir), { force: true });
  } catch {
    // なければ何もしない
  }
}
...
async function loadCache(rootDir) {
  try {
    ...
    return json.version === 2 && json.files ? json.files : {};
  } catch {
    return {};
  }
}
...
async function saveCache(rootDir, files) {
  try {
    ...
    await writeFile(cacheFileFor(rootDir), JSON.stringify({ version: 2, files }));
  } catch {
...
export async function scanLibrary(rootDir, opts = {}) {
  ...
  async function worker() {
    while (index < paths.length) {
      ...
      try {
        ...
        tracks.push({
          id: trackId(rootDir, relPath),
          path: relPath,
          ext,
          size: st.size,
          mtime: Math.floor(st.mtimeMs), // アートワーク URL のキャッシュ切替に使う (Go と同じ切り捨て)
  ...
  // 既定の並び: アーティスト → アルバム → ディスク番号 → トラック番号
  // (複数枚組はディスクごとに 1 から数え直す前提で、ディスク順を先に見る)
